...
def _get_sp_key(sp_id, tenant):
    return sp_id + '.' + tenant


def _get_service_principal_key_entry(sp_id, tenant):
    return {
        'sp_id': sp_id,
        'tenant': tenant
    }
...
class Profile(object):
    ...
    def __init__(self, storage=None, aad_application_factory=None, use_global_creds_cache=True,
        ...
        if use_global_creds_cache:
            ...
            self._creds_cache = CredsCache(self.cli_ctx, self.aad_application_factory)
    ...
    def find_subscriptions_on_login(self,
        ...
        if is_service_principal:
            sp_credential = sp_auth.get_entry_to_persist(username, tenant)
            self._creds_cache.save_service_principal_cred(username, tenant, sp_credential)
    ...
    def get_subscription(self, subscription=None):  # take id or name
        ...
        if not subscriptions:
            app = _create_aad_application(self.cli_ctx, None, self._creds_cache.adal_token_cache)
            accounts = app.get_accounts()
            if accounts:
                token_entry = self.get_token_entry_for_resource(accounts[0]['username'], None, None)
                subscription_finder = SubscriptionFinder(self.cli_ctx,
                                                         self.aad_application_factory,
                                                         self._creds_cache.adal_token_cache)
                subscriptions = subscription_finder.find_from_token_entry(token_entry)
                if subscriptions:
                    consolidated = self._normalize_properties(
                        subscription_finder.user_home_account[_MSAL_ACCOUNT_USERNAME],
                        subscriptions,
                        False,
                        False)
                    self._set_subscriptions(consolidated)
                    subscriptions = deepcopy(consolidated)
        if not subscriptions:
    ...
    def get_token_entry_for_resource(self, username, tenant, resource):
...
class SubscriptionFinder(object):
    ...
    def find_from_token_entry(self, token_entry, resource=None, tenant=None):
        self.user_home_account = self._find_user_home_account(token_entry)
        return self.find_from_access_token(token_entry[_ACCESS_TOKEN], resource, tenant)

    def find_from_access_token(self, access_token, resource=None, tenant=None):
        if tenant is None:
            result = self._find_using_common_tenant(access_token, resource)
        else:
            result = self._find_using_specific_tenant(tenant, access_token)
        return result
...
class CredsCache(object):
    ...
    def __init__(self, cli_ctx, auth_ctx_factory=None):
    ...
    def retrieve_token_for_service_principal(self, sp_id, resource, tenant, use_cert_sn_issuer=False):
        ...
        key_entry = _get_service_principal_key_entry(sp_id, tenant)
        entries = self.adal_token_cache.find(_MSAL_CLIENT_CREDENTIALS_CAT, query=key_entry)
        if not entries:
            raise CLIError("Please run 'az account set' to select active account.")
        password_arg_value = entries[0].get(_SERVICE_PRINCIPAL_SECRET, None) or \
                             entries[0].get(_SERVICE_PRINCIPAL_CERT_FILE, None)
        sp_auth = ServicePrincipalAuth(password_arg_value, use_cert_sn_issuer)
    ...
    def retrieve_secret_of_service_principal(self, sp_id, tenant):
        ...
        return entries[0].get(_SERVICE_PRINCIPAL_SECRET, None)
    ...
    def load_adal_token_cache(self):
        if self._adal_token_cache is None:
            ...
            self._adal_token_cache.key_makers[_MSAL_CLIENT_CREDENTIALS_CAT] = _get_sp_key
        return self._adal_token_cache

    def save_service_principal_cred(self, sp_id, tenant, sp_credential):
        self.load_adal_token_cache()
        key_entry = _get_service_principal_key_entry(sp_id, tenant)
        self._adal_token_cache.modify(_MSAL_CLIENT_CREDENTIALS_CAT, key_entry, sp_credential)
...
class ServicePrincipalAuth(object):
    ...
    def get_entry_to_persist(self, sp_id, tenant):
        entry = _get_service_principal_key_entry(sp_id, tenant)
